...
describe('Simulation Engine', () => {
  ...
  it('changes behavior when retirement age differs only in output metadata', () => {
    ...
    expect(scenario60.states[0].assets.cash.market_value).toBe(
      scenario65.states[0].assets.cash.market_value
    );
  });
  ...
  it('calculates total_assets correctly based on cash and investment', () => {
    ...
    expect(state.metrics.total_assets).toBe(
      state.assets.cash.market_value + state.assets.investment.market_value
    );
  });
